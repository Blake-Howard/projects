  ...
  
{
  ...
  //Here we initialize a list using malloc and that our nodes have a pointer to the LIST node.
  ...
  while(input != 0) {
    ...
    case 0: 
      ...
      input = 0;
      break;
      //create a node at the end to insert a value
    case 1:
...
void append_node(struct CELL *llist, int num) {
  while(llist->LIST != NULL)
    llist = llist->LIST;
  //appending to the end of the list by adding a new node
  llist->LIST = (struct CELL *)malloc(sizeof(struct CELL));
  llist->LIST->number = num;
  llist->LIST->LIST = NULL;
}
...
void delete_node(struct CELL *llist, int num) {
  ...
  if(llist->number == num) {
    ...
    while(llist->LIST->number != num)
      llist = llist->LIST;
    temp = llist->LIST->LIST;
    free(llist->LIST); 
    llist->LIST = temp;
  } 
}
